...
impl SwapFinder for RaydiumV5SwapFinder {
    ...
    fn find_swaps(ix: &Instruction, inner_ixs: &InnerInstructions, account_keys: &Vec<Pubkey>, meta: &TransactionStatusMeta) -> Vec<SwapV2> {
        let exact_in_swaps =  Self::find_swaps_generic(ix, inner_ixs, account_keys, meta, &RAYDIUM_V5_PUBKEY, &[0x8f, 0xbe, 0x5a, 0xda, 0xc4, 0x1e, 0x33, 0xde], 24);
        let exact_out_swaps =  Self::find_swaps_generic(ix, inner_ixs, account_keys, meta, &RAYDIUM_V5_PUBKEY, &[0x37, 0xd9, 0x62, 0x56, 0xa3, 0x4a, 0xb4, 0xad], 24);
        return [exact_in_swaps, exact_out_swaps].concat();
    }
}